...
export class MinioService {
  ...
  async presignedPutUrl(bucketName: string, fileName: string, expiry: number = 24 * 60 * 60) {
    return await this.minioClient.presignedPutObject(bucketName, fileName, expiry);
  }

  async presignedGetUrl(bucketName: string, fileName: string, expiry: number = 24 * 60 * 60) {
    return await this.minioClient.presignedGetObject(bucketName, fileName, expiry);
  }

  async presignedPostPolicy(bucketName: string, fileName: string, expiry: number = 24 * 60 * 60) {
